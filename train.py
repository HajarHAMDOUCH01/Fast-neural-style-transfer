...
def get_style_targets(vgg, style_img):
    """Precompute style target Gram matrics"""
    with torch.no_grad():
        style_features = vgg(style_img)
        style_targets = []
        for feat in style_features:
            style_targets.append(gram_matrix(feat))
...
def train_style_transfer():
    ...
    dataset = COCODataset(root='', transform=transform)
    dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True)
    style_net = StyleTransferNet().to(device)
    vgg = VGG16().to(device)
    vgg.eval()

    style_img = load_style_image('')
    ...
    total_iterations = 0
    target_iterations = 40000  # 2 epochs on 80k images with batch_size=4
